# # backend/utils/compression_handler.py - ✅ 修复版本
# import os
# import logging
...
# import shutil
# import json
# from django.utils import timezone  # ✅ 修复1: 添加 timezone 导入
...
#             print(f"💾 原始视频已保存到: {temp_input_path}")
...
#                         has_audio = True
#                         print(f"🔊 检测到音频流: {stream.get('codec_name')}")
#                         break
...
#             log_file = os.path.join(temp_dir, 'ffmpeg.log')

#             # ✅ 修复2: FFmpeg 命令使用兼容的语法
#             cmd = [
#                 'ffmpeg',
#                 '-i', temp_input_path,
#                 # ✅ 修复: 去掉引号,使用反斜杠转义
...
#             print(f"🎬 执行FFmpeg命令: {' '.join(cmd[:10])}...")
...
#                 print(f"❌ 视频压缩命令执行失败 (返回码: {result.returncode})")
#                 print(f"❌ FFmpeg错误信息:\n{error_msg}")
#                 raise Exception(f"FFmpeg返回码 {result.returncode}: {error_msg}")
...
#                 print(f"⚠️ 视频压缩导致文件变大 ({compressed_size/1024/1024:.2f}MB >= {original_size/1024/1024:.2f}MB)")
#                 print(f"⚠️ 将使用原始文件")
...
#             print(f"❌ 视频压缩超时 {filename} (超过{timeout}秒)")
#             print(f"💡 建议: 增加超时时间或降低视频质量设置")
#             original_file = ContentFile(video_content)
#             original_file.name = filename
#             return original_file, None
...
#     @staticmethod
#     def should_compress_video(video_content, threshold_mb=5.0):  # ✅ 修复4: 从0.5改为5.0
#         """检查视频是否需要压缩"""
