...
const DJANGO_API_ROOT =
  process.env.NODE_ENV === "production"
    ? "https://api.trippalholiday.my/api"
...
const MaterialCard = ({ material, onSelect }) => {
  const typeStyles = {
    ...
    restaurant: {
      ...
      badge: "from-pink-100 to-pink-200 text-pink-800"
    }
  };
...
// ✅ 优化后的详情弹窗组件 - 替换原 Materials.jsx 中的 MaterialDetailModal
const MaterialDetailModal = ({ material, onClose }) => {
  const [activeTab, setActiveTab] = useState('images'); // 'images' | 'videos' | 'info'
  ...
  const fixedDescription = material.description
    ? material.description.replaceAll('/media/', `${DJANGO_API_ROOT.replace('/api', '')}/media/`)
    : "暂无描述";

  const hasImages = material.images?.length > 0;
  const hasVideos = material.videos?.length > 0;
  ...
  return (
    ...
    >
      <div
        className="max-w-7xl w-full bg-white rounded-3xl shadow-2xl overflow-hidden transform transition-all duration-300 scale-100 max-h-[95vh] flex flex-col"
      ...
      >
        ...
        <div className="flex items-center justify-between px-8 py-6 border-b border-gray-200 bg-gradient-to-r from-blue-50 to-indigo-50">
          <div className="flex-1">
            <div className="flex items-center gap-3 mb-2">
              <span className={`px-3 py-1 text-xs font-bold rounded-full ${material.material_type === 'hotel' ? 'bg-blue-100 text-blue-700' :
                  material.material_type === 'ticket' ? 'bg-green-100 text-green-700' :
                    material.material_type === 'route' ? 'bg-purple-100 text-purple-700' :
            ...
            <h3 className="text-3xl font-bold text-gray-900 mb-1">{material.title}</h3>
            {material.price && (
          ...
          <button
            onClick={onClose}
            className="w-12 h-12 flex items-center justify-center rounded-xl hover:bg-white transition-all shadow-sm text-gray-500 hover:text-gray-800"
            aria-label="关闭"
          >
          ...
          <div className="flex items-center gap-2">
            {hasImages && (
              <button
                ...
                className={`px-6 py-3 rounded-xl font-semibold transition-all flex items-center gap-2 ${activeTab === 'images'
            ...
            {hasVideos && (
              <button
                ...
                className={`px-6 py-3 rounded-xl font-semibold transition-all flex items-center gap-2 ${activeTab === 'videos'
              ...
              >
                ...
                视频库 ({material.videos.length})
              </button>
            )}

            <button
              onClick={() => setActiveTab('info')}
              className={`px-6 py-3 rounded-xl font-semibold transition-all flex items-center gap-2 ${activeTab === 'info'
                ...
                >
                  {material.images.map((img) => (
                    <SwiperSlide key={img.id}>
                      <div className="h-full bg-gradient-to-br from-gray-100 to-gray-200 rounded-3xl overflow-hidden relative">
                        <img
                ...
                >
                  {material.videos.map((vid) => (
                    <SwiperSlide key={vid.id}>
                      <div className="h-full bg-black rounded-3xl overflow-hidden relative">
                        <video
                          controls
                          preload="metadata"
                          playsInline
                          ...
                          poster={
                            vid.thumbnail
                              ...
                              : getImageUrl(material.header_image)
                          }
                        >
                        ...
                        {(vid.title || vid.description) && (
                          <div className="absolute bottom-0 left-0 right-0 bg-gradient-to-t from-black/90 to-transparent p-6">
                            {vid.title && <p className="text-white font-bold text-xl mb-2">{vid.title}</p>}
                            {vid.description && <p className="text-gray-300 text-sm">{vid.description}</p>}
                          </div>
                        )}
                      </div>
                    </SwiperSlide>
                    ...
                    <div className="bg-gradient-to-br from-blue-50 via-indigo-50 to-purple-50 rounded-3xl p-8 shadow-lg border border-blue-100">
                      ...
                      <div className="space-y-4">
                        <div className="bg-white rounded-xl p-4 shadow-sm">
                          ...
                          <p className="text-lg font-bold text-gray-900">{material.material_type_display}</p>
                        </div>
                        <div className="bg-white rounded-xl p-4 shadow-sm">
                          <p className="text-xs text-gray-500 mb-1">目的地</p>
                          <p className="text-lg font-bold text-gray-900">{material.destination?.name || "—"}</p>
                        </div>
                        {material.price && (
                          <div className="bg-white rounded-xl p-4 shadow-sm">
                        ...
                        {hasImages && (
                          <div className="bg-white rounded-xl p-4 shadow-sm">
                            ...
                            <p className="text-lg font-bold text-blue-600">{material.images.length} 张</p>
                          </div>
                        )}
                        {hasVideos && (
                          <div className="bg-white rounded-xl p-4 shadow-sm">
                            <p className="text-xs text-gray-500 mb-1">视频数量</p>
                            <p className="text-lg font-bold text-red-600">{material.videos.length} 个</p>
                          </div>
                        )}
                      </div>
                    </div>
        ...
        {/* ✅ 底部操作栏 - 固定 */}
        <div className="px-8 py-6 bg-gradient-to-r from-gray-50 to-white border-t border-gray-200 flex justify-between items-center">
          <div className="text-sm text-gray-500">
            最后更新: {new Date(material.updated_at).toLocaleString('zh-CN')}
          </div>
          <div className="flex gap-3">
            ...
            <a href={`${DJANGO_API_ROOT}/materials/${material.id}/download-pdf/`}
              target="_blank"
              rel="noopener noreferrer"
              className="flex-1 px-4 py-3 bg-gradient-to-r from-green-600 to-emerald-600 text-white rounded-xl font-semibold hover:shadow-lg transform hover:scale-105 transition-all duration-300 flex items-center justify-center gap-2 text-sm"
              title="新版 PDF（支持 emoji）"
            >
              <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 10v6m0 0l-3-3m3 3l3-3m2 8H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" />
              </svg>
